...
def load_initial_data(path):
    ...
    items = [it for it in items if it['human_order']]
    prompts, responses = [it['prompt'] for it in items], [it['response'] for it in items]

    prefix = 'Generate a sentence including the following keywords in the same order as listed: %s\n\nAnswer:'
    concepts = ['-'.join(q.split(prefix.split('%s')[0])[-1].split(prefix.split('%s')[-1])[0].split()) for q in prompts]
    return concepts, prompts, responses
